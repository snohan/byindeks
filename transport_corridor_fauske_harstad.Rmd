...
# Points: map
# Corridor index: per year (this year to date) and LATER combined from first year
# LATER Corridor index: per month
# Point index: map for combined index
# Point index: table for combined index
# LATER Point index: table per year
```
...
points_indices_year_to_date_total <- points_indices_year_to_date %>% 
  dplyr::select(name, road_reference, year, total_index) %>% 
...
points_indices_year_to_date_total_refyear %>%
  select(name, road_reference, starts_with("index")) %>%
  flextable() %>%
