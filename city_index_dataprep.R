...
if(city_number == 959) {source("city_index_rolling_sub_area.R")}
...
# E18 Buskerudbyen ----
if(city_number == 1952) {source("e18_buskerudbyen.R")}
...
city_index_tromso_2019_2022 <-
  readr::read_rds("data_indexpoints_tidy/city_index_tromso_2019_2022.rds") |>
  dplyr::mutate(
...
city_index_final <-
  ...
  dplyr::mutate(area_name = city_name)
...
all_rolling_indexes_chained <-
  ...
  dplyr::select(-index_i_org) |>
  dplyr::relocate(index_i)
...
# YDT for Excel ----
# Not for cities with toll stations!
{
