...
borderdata_2019_to <- "2020-01-06T00:00:00+01:00"

borderdata_2020_from <- "2020-09-28T00:00:00+02:00"
borderdata_2020_to <- "2021-01-04T00:00:00+01:00"
...
dt_2019_lillejulaften <- border_dt %>% 
  filter(year == 2019) %>%
  filter(weekno == 52,
         weekday == 1) %>%
  select(point_id, total_volume, weekno, weekday) %>% 
  mutate(weekday = "Lille julaften")

dt_2019_julaften <- border_dt %>% 
  filter(year == 2019) %>%
  filter(weekno == 52,
         weekday == 2) %>%
  select(point_id, total_volume, weekno, weekday) %>% 
  mutate(weekday = "Julaften")

dt_2019_1juledag <- border_dt %>% 
  filter(year == 2019) %>%
  filter(weekno == 52,
         weekday == 3) %>%
  select(point_id, total_volume, weekno, weekday) %>% 
  mutate(weekday = "1. juledag")

dt_2019_2juledag <- border_dt %>% 
  filter(year == 2019) %>%
  filter(weekno == 52,
         weekday == 4) %>%
  select(point_id, total_volume, weekno, weekday) %>% 
  mutate(weekday = "2. juledag")

dt_2019_3juledag <- border_dt %>% 
  filter(year == 2019) %>%
  filter(weekno == 52,
         weekday == 5) %>%
  select(point_id, total_volume, weekno, weekday) %>% 
  mutate(weekday = "3. juledag")

dt_2019_4juledag <- border_dt %>% 
  filter(year == 2019) %>%
  filter(weekno == 52,
         weekday == 6) %>%
  select(point_id, total_volume, weekno, weekday) %>% 
  mutate(weekday = "4. juledag")

dt_2019_5juledag <- border_dt %>% 
  filter(year == 2019) %>%
  filter(weekno == 52,
         weekday == 7) %>%
  select(point_id, total_volume, weekno, weekday) %>% 
  mutate(weekday = "5. juledag")

dt_2019_6juledag <- border_dt %>% 
  filter(year == 2019) %>%
  filter(weekno == 1,
         weekday == 1) %>%
  select(point_id, total_volume, weekno, weekday) %>% 
  mutate(weekday = "6. juledag")

dt_2019_new_years_eve <- border_dt %>% 
  filter(year == 2019) %>%
  filter(weekno == 1,
         weekday == 2) %>%
  select(point_id, total_volume, weekno, weekday) %>% 
  mutate(weekday = "Nyttårsaften")

dt_2019_new_years_day <- border_dt %>% 
  filter(year == 2020) %>%
  filter(weekno == 1,
         weekday == 3) %>%
  select(point_id, total_volume, weekno, weekday) %>% 
  mutate(weekday = "1. nyttårsdag")

dt_2019_jul <- bind_rows(
  dt_2019_lillejulaften,
  dt_2019_julaften,
  dt_2019_1juledag,
  dt_2019_2juledag,
  dt_2019_3juledag,
  ...
  dt_2019_5juledag,
  dt_2019_6juledag,
  dt_2019_new_years_eve,
  dt_2019_new_years_day
)
...
dt_2020_lillejulaften <- border_dt %>% 
  filter(year == 2020) %>%
  filter(weekno == 52,
         weekday == 3) %>% 
  select(point_id, total_volume, weekno, weekday) %>% 
...
dt_2020_new_years_eve <- border_dt %>% 
  filter(year == 2020) %>%
  filter(weekno == 53,
         weekday == 4) %>% 
  select(point_id, total_volume, weekno, weekday) %>% 
...
dt_2020_new_years_day <- border_dt %>% 
  filter(year == 2021) %>%
  filter(weekno == 53,
         weekday == 5) %>% 
  select(point_id, total_volume, weekno, weekday) %>% 
...
dt_2020_jul <- bind_rows(
  dt_2020_lillejulaften,
  dt_2020_julaften,
  dt_2020_1juledag,
  dt_2020_2juledag,
  dt_2020_3juledag,
  ...
  dt_2020_5juledag,
  dt_2020_6juledag,
  dt_2020_new_years_eve,
  dt_2020_new_years_day
)

dt_2020_53_last <- border_dt %>% 
  filter(year == 2021) %>%
  filter(weekno == 53,
         weekday > 5) %>% 
  select(point_id, total_volume, weekno, weekday) 
...
trp_dt_52 <- calculate_trp_border_index(border_dt_2019_51, dt_2020_52)
border_trp_dt_jul <- calculate_trp_border_index_movable_days(
  dt_2019_jul, dt_2020_jul)
trp_dt_53 <- calculate_trp_border_index(dt_2020_1, dt_2020_53_last)
...
border_dt_52 <- calculate_border_index(
  border_dt_2019_51, 
  dt_2020_52) %>% 
  mutate(periode = "Uke \n 52")
...
city_dt_jul <- inner_join(dt_2019_jul, dt_2020_jul,
                            by = c("point_id", "weekday"),
                            suffix = c("_b", "_c")) %>% 
  group_by(weekday) %>% 
  ...
  mutate(index = round((city_volume_c / city_volume_b - 1) * 100, 
                       digits = 1),
         weekday = factor(weekday, levels = juledager)) %>% 
  select(weekday, index) %>% 
  arrange(weekday) %>% 
  tidyr::pivot_wider(names_from = weekday,
                     values_from = index)

border_dt_53 <- calculate_border_index(dt_2020_1, dt_2020_53_last) %>% 
  mutate(periode = "Uke \n 53")
...
```{r plot_2019_2, fig.width=6, fig.height=5, fig.cap="Estimert endring i trafikk over riksgrensen, uke 46 - 53."}
border_base_2019 %>% 
  dplyr::filter(periode %in% c("Uke \n 46",
                               "Uke \n 47",
                               "Uke \n 48",
...
city_dt_jul %>% 
    ...
    autofit() %>%
    height_all(height = .2) %>%
    set_caption("Prosentvis endring i trafikkmengde ved jul og nyttår.",
              autonum = table_numbers,
              style = "Tabelltekst")
```
...
I tabellene over er endringen i trafikkmengden oppgitt i prosent, sammenlignet med uke tilsvarende uke i 2019 som utgangspunkt. Det vil si at trafikken har gått ned fra år 2019 til samme ukedag i 2020, når prosentverdien er negativ.
...
`r officer::fpar("", officer::run_pagebreak())`
```{r trptable_201952}
make_border_trp_table(trp_dt_52,
                      "Prosentvis endring i trafikkmengde fra uke 51 2019 til uke 52 2020.")
```
...
border_trp_dt_jul %>% 
    ...
    autofit() %>%
    height_all(height = .2) %>%
    set_caption("Prosentvis endring i trafikkmengde ved jul og nyttår, fra 2019 til 2020.",
              autonum = table_numbers,
              style = "Tabelltekst")
```
...
`r officer::fpar("", officer::run_pagebreak())`
```{r trptable_201953}
make_border_trp_table(trp_dt_53,
                      "Prosentvis endring i trafikkmengde fra uke 1 2020 til uke 53 2020.")
```
