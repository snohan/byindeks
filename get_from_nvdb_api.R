...
call_and_parse_nvdb_api <- function(api_query) {
  ...
  respons <-
    httr::GET(
      api_query,
      httr::add_headers(.headers = nvdb_headers)
    )
...
hent_vegpunkt_via_latlon_for_flere_punkter <- function(trp_df) {
  ...
  while (trp_count <= number_of_points) {
    ...
    new_data_point <- data.frame(
      ...
      hent_vegpunkt_via_latlon(
        trp_df$lat[trp_count],
...
get_aadt_by_area <- function(
  ...
  adt <-
    ...
    # thus causing unnest to get incompatible types.
...
get_aadt_by_area_with_all_road_references <-
  ...
  # Purpose: using road reference sections as a proxy for geometry (simplifying matches with other objects)
