...
```{r indeks}
# CSV file is made in "bomdata_trondheim.R"
bomindeks_trondheim_maanedlig <- read.csv2("bom_maanedsindekser.csv",
                                         stringsAsFactors = FALSE) %>% 
  dplyr::mutate(aar_maaned = lubridate::ymd(aar_maaned))
...
stepplot_by_class(bomindeks_trondheim_maanedlig, 1, 10)
```

```{r indeksplott2, fig.width=8, fig.height=8}
stepplot_by_class(bomindeks_trondheim_maanedlig, 11, 20)
```