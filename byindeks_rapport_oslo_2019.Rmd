...
road_lengths <- read_road_length_csv("road_lengths/oslo_road_lengths.csv")

all_point_info_old <- 
  read.csv2("data_indexpoints_tidy/indekspunkt_oslo_2017.csv")

all_point_info <- 
  read.csv2("data_indexpoints_tidy/indekspunkt_oslo_2019.csv")

city_info <-
    read.csv2("data_indexpoints_tidy/byindeks_oslo_2019.csv")

city_monthly <- 
  read.csv2("data_indexpoints_tidy/byindeks_maanedlig_oslo_2019.csv")

# city_36_month <- 
#   read.csv2("data_indexpoints_tidy/byindeks_36_maaneder_oslo_2019.csv")
...
city_info_all_years_old <- city_info %>% 
...
index_all_years_old <- round(city_info_all_years_old$index, digits = 1)
index_all_years_ci_lower_old <- 
  round(city_info_all_years_old$index - 
          city_info_all_years_old$konfidensintervall, digits = 1)
index_all_years_ci_upper_old <- 
  round(city_info_all_years_old$index + 
          city_info_all_years_old$konfidensintervall, digits = 1)
...
Dagens byvekstavtale gjelder for Oslo og kommunene i gamle Akershus fylke. Veglengder fordelt på vegkategori er vist nedenfor.
...
all_point_info_old %>%
  ...
  padding(padding.top = .3,
          padding.bottom = .3) %>%
  set_caption("Trafikkregistreringspunktene som inngikk i byindeksen 2017-2019. ÅDT er oppgitt for lette biler (målt til kortere enn 5,6 m), og gjelder for det første året i avtaleperioden med god datakvalitet.")
```
...
all_point_info %>%
  ...
  padding(padding.top = .3,
          padding.bottom = .3) %>%
  set_caption("Trafikkregistreringspunktene som inngår i byindeksen fra 2020. ÅDT er oppgitt for lette biler (målt til kortere enn 5,6 m), og gjelder for det første året i avtaleperioden med god datakvalitet.")
```


Kartet nedenfor viser plasseringen av trafikkregistreringspunktene i perioden 2017-2019.

```{r map_trp_old}
create_point_adt_map(all_point_info_old)
```

Kartet nedenfor viser plasseringen av trafikkregistreringspunktene i perioden fra 2020.
...
```{r plot_city_index_old}
set.seed(123) # To keep the jittering the same for each render of the plot

city_info_each_one_year_and_total_old <- city_info %>% 
  dplyr::filter(year != "2019-2020") %>% 
  dplyr::filter(year != "2017-2020")

ggplot() +
  geom_jitter(data = all_point_info_long_old,
              aes(year, index_value),
              color = "#ED9300", size = 2, alpha = 0.25, width = 0.1) +
  geom_hline(yintercept = 0, alpha = 0.3) +
  geom_segment(data = city_info_each_one_year_and_total_old,
               aes(x = year, xend = year,
                   y = 0, yend = index),
               color = "#ED9300", size = 0.6, alpha = 0.9) +
  geom_point(data = city_info_each_one_year_and_total_old,
             aes(year, index),
...
```{r plot_city_index}
set.seed(132) # To keep the jittering the same for each render of the plot
...
Kartet nedenfor viser endringen i byindekspunktene fra referanseåret 2017 til og med 2019.

```{r map_trp_index_old}
create_pointindex_map(all_point_info_old)
```

Kartet nedenfor viser endringen i byindekspunktene fra referanseåret 2019 til og med 2020.
...
```{r point_index_table_old}
index_column_names_old <- all_point_info_old %>% 
...
all_point_info_old %>%
  ...
  #select(-index_20) %>% 
  flextable() %>%
  colformat_num(j = index_column_names_old,
                digits = 1) %>%
  set_header_labels(name = "Navn",
...
I figuren nedenfor er spredningen i punktindeksene illustrert. Den horisontale grønne streken viser byindeksens samlede verdi for perioden.


```{r point_graph_old, fig.width=8, fig.height=9}
all_point_info_index_old <- all_point_info_old %>% 
  dplyr::filter(!is.na(index)) 

ggplot2::ggplot() +
  geom_point(data = all_point_info_index_old, 
             aes(x = reorder(name, index), y = index, size = adt),
             color = "#ED9300", alpha = 0.6) +
  geom_hline(yintercept = index_all_years_old, color = "#58B02C") +
  ...
  geom_rect(aes(xmin = -Inf, xmax = Inf, 
                ymin = index_all_years_ci_lower_old, 
                ymax = index_all_years_ci_upper_old),
...
```{r point_graph, fig.width=8, fig.height=9}
