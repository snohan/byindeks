...
points_er <- points_first_commission_in_trafikkdata %>% 
  ...
  dplyr::filter(road_category %in% c("E", "R"),
                is_visible == TRUE) %>% 
  dplyr::select(trp_id, trp_name, road_category, road_reference, county_name, operational_status,
...
points <- dplyr::bind_rows(points_er#, 
                           #points_f
                           )
```
...
mdts_prepared <- mdts %>% 
  ...
  dplyr::mutate(month_object = lubridate::make_date(year = year, month = month),
                ...
                tertial = dplyr::case_when(month < 5 ~ 1,
                                           month < 9 ~ 2,
                                           TRUE ~ 3))
...
write.csv(trp_mdt, file = "spesialuttak/trp_mdt_2021-08.csv", row.names = F)
```
