---
title: Daglig endring i trafikk ved riksgrensen
output: 
  word_document:
    reference_docx: svv_notatmal3.docx
    toc: true
    toc_depth: 2
    fig_caption: true
    fig_width: 7
toc_title: "Innhold"
---

```{r setup, include = FALSE, echo = FALSE, warning=FALSE, message=FALSE}
# Packages are loaded through sourcing rmd_setup.R
source("rmd_setup.R")

# Traffic Data API calls to get points metadata and aadt
source("get_from_trafficdata_api.R")

# Points on border crossings
border_trp_ids <- c("04904V971774",
                    "35229V971507",
                    "02535V971411",
                    "09269V971425",
                    "21405V2607269",
                    "05732V971567",
                    "76778V704564",
                    "57929V705247",
                    "94299V704696",
                    "94864V704707",
                    "00737V704646",
                    "14158V705081",
                    "69140V704643",
                    "11051V704737",
                    "66220V72824",
                    "84237V578097",
                    "50089V578151",
                    "93561V578187",
                    "99923V578123",
                    "08581V885541",
                    "35829V885266",
                    "77275V885276",
                    "01777V885181",
                    "45313V1125788",
                    "85233V930344",
                    "13177V930320",
                    "97351V930305",
                    "99312V930442",
                    "68101V930654",
                    "97687V930620"
                    )

# All points from Traffic Data API
points <- get_points() %>%
  dplyr::select(trp_id, name, road_reference, municipality_name,
                lat, lon) %>%
  dplyr::distinct(trp_id, .keep_all = T)

# Adding metadata to trps
border_trps <- points %>%
  dplyr::filter(trp_id %in% border_trp_ids) %>% 
  dplyr::arrange(desc(lat))


# TRP AADT ####
adt <- get_aadt_for_trp_list(border_trps$trp_id)

adt_filtered <- adt %>%
  #dplyr::filter(coverage > 20) %>%
  dplyr::group_by(trp_id) %>%
  dplyr::filter(year == max(year)) %>%
  dplyr::select(trp_id, adt)

border_trp_adt <- border_trps %>% 
  left_join(adt_filtered)


# Daily traffic ####
# Different comparisons with different bases:
# 1. Week 10 2020
# 2. Same week in 2019
# 3. Easter 2019 to Easter 2020

# From week 11 to week 35
borderdata_2019_from_1 <- "2019-03-11T00:00:00+01:00"
borderdata_2019_to_1 <- "2019-06-01T00:00:00+01:00"

borderdata_2019_from_2 <- "2019-06-01T00:00:00+01:00"
borderdata_2019_to_2 <- "2019-09-02T00:00:00+01:00"

# From week 10 to week 35
borderdata_2020_from <- "2020-03-02T00:00:00+01:00"
borderdata_2020_to <- "2020-08-31T00:00:00+01:00"

ukedager <- c("Mandag",
              "Tirsdag",
              "Onsdag",
              "Torsdag",
              "Fredag",
              "Lørdag",
              "Søndag")

norske_ukedager <- data.frame(weekday = c(1, 2, 3, 4, 5, 6, 7),
                              ukedag = ordered(ukedager,
                                               levels = ukedager))

# Fetch daily traffic from API
border_dt_2019_1 <- get_dt_for_trp_list(border_trps$trp_id,
                                      borderdata_2019_from_1,
                                      borderdata_2019_to_1)

border_dt_2019_2 <- get_dt_for_trp_list(border_trps$trp_id,
                                      borderdata_2019_from_2,
                                      borderdata_2019_to_2)

border_dt_2019 <- bind_rows(border_dt_2019_1, border_dt_2019_2)

border_dt_2020 <- get_dt_for_trp_list(border_trps$trp_id,
                                      borderdata_2020_from,
                                      borderdata_2020_to)


border_dt <- bind_rows(border_dt_2019, border_dt_2020) %>%
  select(-point_name) %>%
  mutate(year = lubridate::year(from),
         weekno = lubridate::isoweek(from),
         weekday = lubridate::wday(from,
                                   week_start =
                                     getOption("lubridate.week.start", 1))) %>%
  select(-from) %>% 
  filter(coverage > 99)



# Weeks 2019 ####
border_dt_2019_11 <- border_dt %>% 
  filter(year == 2019) %>%
  filter(weekno == 11) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2019_12 <- border_dt %>% 
  filter(year == 2019) %>%
  filter(weekno == 12) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2019_13 <- border_dt %>% 
  filter(year == 2019) %>%
  filter(weekno == 13) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2019_14 <- border_dt %>% 
  filter(year == 2019) %>%
  filter(weekno == 14) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2019_15_palmweekend <- border_dt %>% 
  filter(year == 2019) %>%
  filter(weekno == 15,
         weekday > 4) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2019_16_easterweek <- border_dt %>% 
  filter(year == 2019) %>%
  filter(weekno == 16) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2019_17_eastermonday <- border_dt %>% 
  filter(year == 2019) %>%
  filter(weekno == 17,
         weekday < 2) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2019_17 <- border_dt %>% 
  filter(year == 2019) %>%
  filter(weekno == 17,
         weekday > 1) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2019_18 <- border_dt %>% 
  filter(year == 2019) %>%
  filter(weekno == 18) %>% 
  # removing 1 May
  filter(weekday != 3) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2019_1_may <- border_dt %>% 
  filter(year == 2019) %>%
  filter(weekno == 18) %>% 
  filter(weekday == 3) %>% 
  select(point_id, total_volume, weekno, weekday) %>% 
  mutate(weekday = "1. mai")

border_dt_2019_19 <- border_dt %>% 
  filter(year == 2019) %>%
  filter(weekno == 19) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2019_20 <- border_dt %>% 
  filter(year == 2019) %>%
  filter(weekno == 20) %>% 
  # removing 17 May
  filter(weekday != 5) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2019_17_may <- border_dt %>% 
  filter(year == 2019) %>%
  filter(weekno == 20) %>% 
  filter(weekday == 5) %>% 
  select(point_id, total_volume, weekno, weekday) %>% 
  mutate(weekday = "17. mai")

border_dt_2019_21 <- border_dt %>% 
  filter(year == 2019) %>%
  filter(weekno == 21) %>% 
  select(point_id, total_volume, weekno, weekday)

# Special days 2019
border_dt_2019_spring_specials <-
  dplyr::bind_rows(border_dt_2019_1_may,
                   border_dt_2019_17_may)

border_dt_2019_22_ascension <- border_dt %>% 
  filter(year == 2019) %>%
  filter(weekno == 22) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2019_23_whitsun <- border_dt %>% 
  filter(year == 2019) %>%
  filter(weekno == 23) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2019_24_whitmonday <- border_dt %>% 
  filter(year == 2019) %>%
  filter(weekno == 24) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2019_25 <- border_dt %>% 
  filter(year == 2019) %>%
  filter(weekno == 25) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2019_26 <- border_dt %>% 
  filter(year == 2019) %>%
  filter(weekno == 26) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2019_27 <- border_dt %>% 
  filter(year == 2019) %>%
  filter(weekno == 27) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2019_28 <- border_dt %>% 
  filter(year == 2019) %>%
  filter(weekno == 28) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2019_29 <- border_dt %>% 
  filter(year == 2019) %>%
  filter(weekno == 29) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2019_30 <- border_dt %>% 
  filter(year == 2019) %>%
  filter(weekno == 30) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2019_31 <- border_dt %>% 
  filter(year == 2019) %>%
  filter(weekno == 31) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2019_32 <- border_dt %>% 
  filter(year == 2019) %>%
  filter(weekno == 32) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2019_33 <- border_dt %>% 
  filter(year == 2019) %>%
  filter(weekno == 33) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2019_34 <- border_dt %>% 
  filter(year == 2019) %>%
  filter(weekno == 34) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2019_35 <- border_dt %>% 
  filter(year == 2019) %>%
  filter(weekno == 35) %>% 
  select(point_id, total_volume, weekno, weekday)

# ADD NEW WEEK HERE!!!



# Weeks 2020 ####
border_dt_2020_10 <- border_dt %>% 
  filter(year == 2020) %>%
  filter(weekno == 10) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2020_11 <- border_dt %>% 
  filter(year == 2020) %>%
  filter(weekno == 11) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2020_12 <- border_dt %>% 
  filter(year == 2020) %>%
  filter(weekno == 12) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2020_13 <- border_dt %>% 
  filter(year == 2020) %>%
  filter(weekno == 13) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2020_14 <- border_dt %>% 
  filter(year == 2020) %>%
  filter(weekno == 14,
         weekday < 5) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2020_14_palmweekend <- border_dt %>% 
  filter(year == 2020) %>%
  filter(weekno == 14,
         weekday > 4) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2020_15_easterweek <- border_dt %>% 
  filter(year == 2020) %>%
  filter(weekno == 15) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2020_16_eastermonday <- border_dt %>% 
  filter(year == 2020) %>%
  filter(weekno == 16,
         weekday < 2) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2020_16 <- border_dt %>% 
  filter(year == 2020) %>%
  filter(weekno == 16,
         weekday > 1) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2020_17 <- border_dt %>% 
  filter(year == 2020) %>%
  filter(weekno == 17) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2020_18 <- border_dt %>% 
  filter(year == 2020) %>%
  filter(weekno == 18) %>%
  # removing 1 May
  filter(weekday != 5) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2020_1_may <- border_dt %>% 
  filter(year == 2020) %>%
  filter(weekno == 18) %>%
  filter(weekday == 5) %>% 
  select(point_id, total_volume, weekno, weekday) %>% 
  mutate(weekday = "1. mai")

border_dt_2020_19 <- border_dt %>% 
  filter(year == 2020) %>%
  filter(weekno == 19) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2020_20 <- border_dt %>% 
  filter(year == 2020) %>%
  filter(weekno == 20) %>% 
  # removing 17 May
  filter(weekday != 7) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2020_17_may <- border_dt %>% 
  filter(year == 2020) %>%
  filter(weekno == 20) %>%
  filter(weekday == 7) %>% 
  select(point_id, total_volume, weekno, weekday) %>% 
  mutate(weekday = "17. mai")

# Special days 2020
border_dt_2020_spring_specials <-
  dplyr::bind_rows(border_dt_2020_1_may,
                   border_dt_2020_17_may)

border_dt_2020_21_ascension <- border_dt %>% 
  filter(year == 2020) %>%
  filter(weekno == 21) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2020_22_whitsun <- border_dt %>% 
  filter(year == 2020) %>%
  filter(weekno == 22) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2020_23_whitmonday <- border_dt %>% 
  filter(year == 2020) %>%
  filter(weekno == 23) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2020_24 <- border_dt %>% 
  filter(year == 2020) %>%
  filter(weekno == 24) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2020_25 <- border_dt %>% 
  filter(year == 2020) %>%
  filter(weekno == 25) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2020_26 <- border_dt %>% 
  filter(year == 2020) %>%
  filter(weekno == 26) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2020_27 <- border_dt %>% 
  filter(year == 2020) %>%
  filter(weekno == 27) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2020_28 <- border_dt %>% 
  filter(year == 2020) %>%
  filter(weekno == 28) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2020_29 <- border_dt %>% 
  filter(year == 2020) %>%
  filter(weekno == 29) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2020_30 <- border_dt %>% 
  filter(year == 2020) %>%
  filter(weekno == 30) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2020_31 <- border_dt %>% 
  filter(year == 2020) %>%
  filter(weekno == 31) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2020_32 <- border_dt %>% 
  filter(year == 2020) %>%
  filter(weekno == 32) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2020_33 <- border_dt %>% 
  filter(year == 2020) %>%
  filter(weekno == 33) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2020_34 <- border_dt %>% 
  filter(year == 2020) %>%
  filter(weekno == 34) %>% 
  select(point_id, total_volume, weekno, weekday)

border_dt_2020_35 <- border_dt %>% 
  filter(year == 2020) %>%
  filter(weekno == 35) %>% 
  select(point_id, total_volume, weekno, weekday)

# ADD NEW WEEK HERE!!!


# Trp index ####
calculate_trp_border_index <- function(dt_base, dt_calc) {
  trp_daily_index <- inner_join(dt_base, dt_calc,
                                by = c("point_id", "weekday"),
                                suffix = c("_b", "_c")) %>% 
    mutate(index = round((total_volume_c / total_volume_b - 1) * 100, 
                         digits = 1),
           index = dplyr::na_if(index, "Inf")) %>% 
    left_join(norske_ukedager) %>% 
    select(point_id, ukedag, index) %>% 
    left_join(points, by = c("point_id" = "trp_id")) %>% 
    left_join(adt_filtered, by = c("point_id" = "trp_id")) %>% 
    tidyr::separate(road_reference, c("road", "strekning", "meter"),
                    sep = " ") %>%    
#    mutate(road = stringr::str_split_fixed(road_reference, " ", 2)) %>% 
    select(name, road, adt, municipality_name, ukedag, index) %>% 
    arrange(factor(ukedag, levels = ukedager)) %>% 
    # TODO: sort by weekday
    tidyr::pivot_wider(names_from = ukedag,
                       values_from = index)
}

calculate_trp_border_index_movable_days <- function(dt_base, dt_calc) {
  trp_daily_index <- inner_join(dt_base, dt_calc,
                            by = c("point_id", "weekday"),
                            suffix = c("_b", "_c")) %>% 
  mutate(index = round((total_volume_c / total_volume_b - 1) * 100, digits = 1),
         index = dplyr::na_if(index, "Inf")) %>% 
  #left_join(norske_ukedager) %>% 
  select(point_id, weekday, index) %>% 
  left_join(points, by = c("point_id" = "trp_id")) %>% 
  left_join(adt_filtered, by = c("point_id" = "trp_id")) %>% 
  tidyr::separate(road_reference, c("road", "strekning", "meter"),
                  sep = " ") %>%
  select(name, road, adt, municipality_name, weekday, index) %>% 
    #arrange(factor(ukedag, levels = ukedager)) %>% 
    # TODO: sort by weekday
    tidyr::pivot_wider(names_from = weekday,
                       values_from = index)
}



# Compare dt to 2020 week 10 ####
border_trp_dt_2020_10_11 <- calculate_trp_border_index(
  border_dt_2020_10,
  border_dt_2020_11) 

border_trp_dt_2020_10_12 <- calculate_trp_border_index(
  border_dt_2020_10, 
  border_dt_2020_12)

border_trp_dt_2020_10_13 <- calculate_trp_border_index(
  border_dt_2020_10, 
  border_dt_2020_13)

border_trp_dt_2020_10_14 <- calculate_trp_border_index(
  border_dt_2020_10, 
  border_dt_2020_14)

border_trp_dt_2020_10_16 <- calculate_trp_border_index(
  border_dt_2020_10, 
  border_dt_2020_16)

border_trp_dt_2020_10_17 <- calculate_trp_border_index(
  border_dt_2020_10, 
  border_dt_2020_17)

border_trp_dt_2020_10_18 <- calculate_trp_border_index(
  border_dt_2020_10, 
  border_dt_2020_18)

border_trp_dt_2020_10_19 <- calculate_trp_border_index(
  border_dt_2020_10, 
  border_dt_2020_19)

border_trp_dt_2020_10_20 <- calculate_trp_border_index(
  border_dt_2020_10, 
  border_dt_2020_20)

# Ending this series with week 20 here. Week 10 is no longer the norm.



# Compare this week to same week 2019 ####
border_trp_dt_201911_202011 <- calculate_trp_border_index(
  border_dt_2019_11, 
  border_dt_2020_11)

border_trp_dt_201912_202012 <- calculate_trp_border_index(
  border_dt_2019_12, 
  border_dt_2020_12)

border_trp_dt_201913_202013 <- calculate_trp_border_index(
  border_dt_2019_13, 
  border_dt_2020_13)

border_trp_dt_201914_202014 <- calculate_trp_border_index(
  border_dt_2019_14, 
  border_dt_2020_14)

border_trp_dt_201917_202017 <- calculate_trp_border_index(
  border_dt_2019_17, 
  border_dt_2020_17)

border_trp_dt_201918_202018 <- calculate_trp_border_index(
  border_dt_2019_18, 
  border_dt_2020_18)

border_trp_dt_201919_202019 <- calculate_trp_border_index(
  border_dt_2019_19, 
  border_dt_2020_19)

border_trp_dt_201920_202020 <- calculate_trp_border_index(
  border_dt_2019_20, 
  border_dt_2020_20)

border_trp_dt_2019_2020_ascension <- calculate_trp_border_index(
  border_dt_2019_22_ascension,
  border_dt_2020_21_ascension
)

border_trp_dt_spring_specials <- calculate_trp_border_index_movable_days(
  border_dt_2019_spring_specials,
  border_dt_2020_spring_specials
)

border_trp_dt_2019_2020_whitsun <- calculate_trp_border_index(
  border_dt_2019_23_whitsun,
  border_dt_2020_22_whitsun
)

border_trp_dt_2019_2020_whitmonday <- calculate_trp_border_index(
  border_dt_2019_24_whitmonday,
  border_dt_2020_23_whitmonday
)

border_trp_dt_201921_202024 <- calculate_trp_border_index(
  border_dt_2019_21,
  border_dt_2020_24
)

border_trp_dt_201925_202025 <- calculate_trp_border_index(
  border_dt_2019_25,
  border_dt_2020_25
)

border_trp_dt_201926_202026 <- calculate_trp_border_index(
  border_dt_2019_26,
  border_dt_2020_26
)

border_trp_dt_201927_202027 <- calculate_trp_border_index(
  border_dt_2019_27,
  border_dt_2020_27
)

border_trp_dt_201928_202028 <- calculate_trp_border_index(
  border_dt_2019_28,
  border_dt_2020_28
)

border_trp_dt_201929_202029 <- calculate_trp_border_index(
  border_dt_2019_29,
  border_dt_2020_29
)

border_trp_dt_201930_202030 <- calculate_trp_border_index(
  border_dt_2019_30,
  border_dt_2020_30
)

border_trp_dt_201931_202031 <- calculate_trp_border_index(
  border_dt_2019_31,
  border_dt_2020_31
)

border_trp_dt_201932_202032 <- calculate_trp_border_index(
  border_dt_2019_32,
  border_dt_2020_32
)

border_trp_dt_201933_202033 <- calculate_trp_border_index(
  border_dt_2019_33,
  border_dt_2020_33
)

border_trp_dt_201934_202034 <- calculate_trp_border_index(
  border_dt_2019_34,
  border_dt_2020_34
)

border_trp_dt_201935_202035 <- calculate_trp_border_index(
  border_dt_2019_35,
  border_dt_2020_35
)


# ADD NEW TRP INDEX HERE!!!

# Compare Easter ####
border_trp_dt_palmweekend <- calculate_trp_border_index(
  border_dt_2019_15_palmweekend, 
  border_dt_2020_14_palmweekend)

border_trp_dt_easterweek <- calculate_trp_border_index(
  border_dt_2019_16_easterweek,
  border_dt_2020_15_easterweek)

border_trp_dt_eastermonday <- calculate_trp_border_index(
  border_dt_2019_17_eastermonday,
  border_dt_2020_16_eastermonday)



# Border index ####
calculate_border_index <- function(dt_base, dt_calc) {
  border_daily_index <- inner_join(dt_base, dt_calc,
                            by = c("point_id", "weekday"),
                            suffix = c("_b", "_c")) %>% 
  group_by(weekday) %>% 
  summarise(border_volume_b = sum(total_volume_b),
            border_volume_c = sum(total_volume_c),
            no_trp = n()
            ) %>% 
  mutate(index = round((border_volume_c / border_volume_b - 1) * 100, 
                       digits = 1)) %>% 
  left_join(norske_ukedager) %>% 
  select(ukedag, no_trp, index)
}

calculate_border_index_movable_days <- function(dt_base, dt_calc) {
  border_daily_index <- inner_join(dt_base, dt_calc,
                            by = c("point_id", "weekday"),
                            suffix = c("_b", "_c")) %>% 
  group_by(weekday) %>% 
  summarise(border_volume_b = sum(total_volume_b),
            border_volume_c = sum(total_volume_c),
            no_trp = n()
            ) %>% 
  mutate(index = round((border_volume_c / border_volume_b - 1) * 100, 
                       digits = 1)) %>% 
  #left_join(norske_ukedager) %>% 
  select(weekday, no_trp, index)
}



# Compare this week to 2020 week 10 ####
border_dt_2020_10_11 <- calculate_border_index(
  border_dt_2020_10, 
  border_dt_2020_11) %>% 
  mutate(periode = "Uke 10 \n til 11")

border_dt_2020_10_12 <- calculate_border_index(
  border_dt_2020_10, 
  border_dt_2020_12) %>% 
  mutate(periode = "Uke 10 \n til 12")

border_dt_2020_10_13 <- calculate_border_index(
  border_dt_2020_10, 
  border_dt_2020_13) %>% 
  mutate(periode = "Uke 10 \n til 13")

border_dt_2020_10_14 <- calculate_border_index(
  border_dt_2020_10, 
  border_dt_2020_14) %>% 
  mutate(periode = "Uke 10 \n til 14")

border_dt_2020_10_16 <- calculate_border_index(
  border_dt_2020_10, 
  border_dt_2020_16) %>% 
  mutate(periode = "Uke 10 \n til 16")

border_dt_2020_10_17 <- calculate_border_index(
  border_dt_2020_10, 
  border_dt_2020_17) %>% 
  mutate(periode = "Uke 10 \n til 17")

border_dt_2020_10_18 <- calculate_border_index(
  border_dt_2020_10, 
  border_dt_2020_18) %>% 
  mutate(periode = "Uke 10 \n til 18")

border_dt_2020_10_19 <- calculate_border_index(
  border_dt_2020_10, 
  border_dt_2020_19) %>% 
  mutate(periode = "Uke 10 \n til 19")

border_dt_2020_10_20 <- calculate_border_index(
  border_dt_2020_10, 
  border_dt_2020_20) %>% 
  mutate(periode = "Uke 10 \n til 20")

border_base_2020 <- bind_rows(border_dt_2020_10_11,
                              border_dt_2020_10_12,
                              border_dt_2020_10_13,
                              border_dt_2020_10_14,
                              border_dt_2020_10_16,
                              border_dt_2020_10_17,
                              border_dt_2020_10_18,
                              border_dt_2020_10_19,
                              border_dt_2020_10_20)

# Ending this series here.



# Compare this week to same week 2019 ####
border_dt_201911_202011 <- calculate_border_index(
  border_dt_2019_11, 
  border_dt_2020_11) %>% 
  mutate(periode = "Uke \n 11")

border_dt_201912_202012 <- calculate_border_index(
  border_dt_2019_12, 
  border_dt_2020_12) %>% 
  mutate(periode = "Uke \n 12")

border_dt_201913_202013 <- calculate_border_index(
  border_dt_2019_13, 
  border_dt_2020_13) %>% 
  mutate(periode = "Uke \n 13")

border_dt_201914_202014 <- calculate_border_index(
  border_dt_2019_14, 
  border_dt_2020_14) %>% 
  mutate(periode = "Uke \n 14")

border_dt_201917_202017 <- calculate_border_index(
  border_dt_2019_17, 
  border_dt_2020_17) %>% 
  mutate(periode = "Uke \n 17")

border_dt_201918_202018 <- calculate_border_index(
  border_dt_2019_18, 
  border_dt_2020_18) %>% 
  mutate(periode = "Uke \n 18")

border_dt_201919_202019 <- calculate_border_index(
  border_dt_2019_19, 
  border_dt_2020_19) %>% 
  mutate(periode = "Uke \n 19")

border_dt_201920_202020 <- calculate_border_index(
  border_dt_2019_20, 
  border_dt_2020_20) %>% 
  mutate(periode = "Uke \n 20")

border_dt_201922_202021 <- calculate_border_index(
  border_dt_2019_22_ascension, 
  border_dt_2020_21_ascension) %>% 
  mutate(periode = "Uke \n 21/22")

border_dt_201923_202022 <- calculate_border_index(
  border_dt_2019_23_whitsun, 
  border_dt_2020_22_whitsun) %>% 
  mutate(periode = "Uke \n 22/23")

border_dt_201924_202023 <- calculate_border_index(
  border_dt_2019_24_whitmonday, 
  border_dt_2020_23_whitmonday) %>% 
  mutate(periode = "Uke \n 23/24")

border_dt_201921_202024 <- calculate_border_index(
  border_dt_2019_21, 
  border_dt_2020_24) %>% 
  mutate(periode = "Uke \n 24/21")

border_dt_201925_202025 <- calculate_border_index(
  border_dt_2019_25, 
  border_dt_2020_25) %>% 
  mutate(periode = "Uke \n 25")

border_dt_201926_202026 <- calculate_border_index(
  border_dt_2019_26, 
  border_dt_2020_26) %>% 
  mutate(periode = "Uke \n 26")

border_dt_201927_202027 <- calculate_border_index(
  border_dt_2019_27, 
  border_dt_2020_27) %>% 
  mutate(periode = "Uke \n 27")

border_dt_201928_202028 <- calculate_border_index(
  border_dt_2019_28, 
  border_dt_2020_28) %>% 
  mutate(periode = "Uke \n 28")

border_dt_201929_202029 <- calculate_border_index(
  border_dt_2019_29, 
  border_dt_2020_29) %>% 
  mutate(periode = "Uke \n 29")

border_dt_201930_202030 <- calculate_border_index(
  border_dt_2019_30, 
  border_dt_2020_30) %>% 
  mutate(periode = "Uke \n 30")

border_dt_201931_202031 <- calculate_border_index(
  border_dt_2019_31, 
  border_dt_2020_31) %>% 
  mutate(periode = "Uke \n 31")

border_dt_201932_202032 <- calculate_border_index(
  border_dt_2019_32, 
  border_dt_2020_32) %>% 
  mutate(periode = "Uke \n 32")

border_dt_201933_202033 <- calculate_border_index(
  border_dt_2019_33, 
  border_dt_2020_33) %>% 
  mutate(periode = "Uke \n 33")

border_dt_201934_202034 <- calculate_border_index(
  border_dt_2019_34, 
  border_dt_2020_34) %>% 
  mutate(periode = "Uke \n 34")

border_dt_201935_202035 <- calculate_border_index(
  border_dt_2019_35, 
  border_dt_2020_35) %>% 
  mutate(periode = "Uke \n 35")






# ADD NEW WEEK HERE!

border_base_2019 <- bind_rows(border_dt_201911_202011,
                              border_dt_201912_202012,
                              border_dt_201913_202013,
                              border_dt_201914_202014,
                              border_dt_201917_202017,
                              border_dt_201918_202018,
                              border_dt_201919_202019,
                              border_dt_201920_202020,
                              border_dt_201922_202021,
                              border_dt_201923_202022,
                              border_dt_201924_202023,
                              border_dt_201921_202024)

border_base_2019_part_2 <- bind_rows(border_dt_201925_202025,
                                     border_dt_201926_202026,
                                     border_dt_201927_202027,
                                     border_dt_201928_202028,
                                     border_dt_201929_202029,
                                     border_dt_201930_202030,
                                     border_dt_201931_202031,
                                     border_dt_201932_202032,
                                     border_dt_201933_202033,
                                     border_dt_201934_202034,
                                     border_dt_201935_202035)


# Compare spring specials ####
border_dt_spring_specials <- calculate_border_index_movable_days(
  border_dt_2019_spring_specials,
  border_dt_2020_spring_specials) %>% 
  mutate(periode = "Spesielle dager") %>% 
  rename(ukedag = weekday)

# Compare Easter ####
border_dt_palmweekend <- calculate_border_index(
  border_dt_2019_15_palmweekend, 
  border_dt_2020_14_palmweekend) %>% 
  mutate(periode = "Palmehelga")

border_dt_easterweek <- calculate_border_index(
  border_dt_2019_16_easterweek, 
  border_dt_2020_15_easterweek) %>% 
  mutate(periode = "Påskeuka")

border_dt_eastermonday <- calculate_border_index(
  border_dt_2019_17_eastermonday, 
  border_dt_2020_16_eastermonday) %>% 
  mutate(periode = "2. påskedag")

border_easter <- bind_rows(border_dt_palmweekend,
                           border_dt_easterweek,
                           border_dt_eastermonday
                           ) %>% 
  mutate(periode = factor(periode,
                          levels = c("Palmehelga", "Påskeuka", "2. påskedag")))

# Function for generating table
make_border_table <- function(border_base, caption_text) {
  
  border_base_table_ready <- border_base %>%
    dplyr::ungroup() %>% 
    dplyr::select(-no_trp) %>%
    dplyr::mutate(index = dplyr::na_if(index, "Inf")) %>% 
    tidyr::pivot_wider(names_from = ukedag,
                       values_from = index) %>% 
    dplyr::mutate(periode = stringr::str_replace(periode, " \n", ""))
  
  ncol_dt <- ncol(border_base_table_ready)
  
  border_table <- border_base_table_ready %>% 
    flextable() %>%
    set_header_labels(periode = "Sammenlignet") %>%
    bold(part = "header") %>%
    fontsize(size = 9, part = "all") %>%
    font(fontname = "Lucida Sans Unicode", part = "all") %>%
    bg(bg = "#ED9300", part = "header") %>%
    border_remove() %>%
    hline_top(part = "header", border = borderline) %>%
    hline_bottom(part = "all", border = borderline) %>%
    colformat_num(j = 2:ncol_dt, na_str = "", digits = 1) %>% 
    autofit() %>%
    height_all(height = .2) %>%
    padding(padding.top = .3,
            padding.bottom = .3) %>%
    set_caption(caption_text)
  
  return(border_table)
}

make_border_trp_table <- function(border_trp_dt, caption_text) {
  
  border_trp_dt_table_ready <- border_trp_dt %>%
    select(-road, -adt, -municipality_name)
  
  ncol_dt <- ncol(border_trp_dt_table_ready)
  
  border_trp_table <- border_trp_dt_table_ready %>% 
    flextable() %>%
    set_header_labels(name = "Punktnavn") %>%
    bold(part = "header") %>%
    fontsize(size = 9, part = "all") %>%
    font(fontname = "Lucida Sans Unicode", part = "all") %>%
    bg(bg = "#ED9300", part = "header") %>%
    border_remove() %>%
    hline_top(part = "header", border = borderline) %>%
    hline_bottom(part = "all", border = borderline) %>%
    colformat_num(j = 2:ncol_dt, na_str = "", digits = 1) %>% 
    autofit() %>%
    height_all(height = .2) %>%
    padding(padding.top = .3,
            padding.bottom = .3) %>%
    set_caption(caption_text)
  
  return(border_trp_table)
}

make_daily_border_plot <- function(area_dt, subtitle_text) {
  
  area_dt %>%
  ggplot2::ggplot(aes(ukedag, index)) +
  geom_bar(aes(fill = index > 0), stat = "identity",
           color = "#000000",
           alpha = 0.6) +
  scale_fill_manual(guide = FALSE, breaks = c(FALSE, TRUE),
                    values = c("#ed1c2e", "#58b02c")) +
  geom_hline(yintercept = 0, color = "#000000") +
  facet_grid(periode ~., switch = "y") +
  scale_y_continuous(position = "right",
                     limits = c(-100, 100),
                     breaks = c(-80, 0, 80)) +
  xlab("") +
  ylab("Endring i trafikkmengde (%)\n") +
  ggtitle(label = "Daglig endring i trafikkmengde over riksgrensen",
          subtitle = subtitle_text) +
  theme(strip.text.y = element_text(angle = 180),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_blank(),
        strip.background = element_blank()) +
  labs(caption = "Data: Statens vegvesen")
}
```

# Bakgrunn

Som følge av smitteverntiltak i forbindelse med spredningen av det nye koronaviruset SARS-CoV-2, er det interessant å se hvordan trafikken endret seg. De første tiltakene ble annonsert av Regjeringen torsdag 12. mars 2020 (uke 11), og trådde i kraft fredag 13. mars.

Statens vegvesen har i dag til sammen 30 trafikkregistreringspunkter ved de ulike riksgrenseovergangene fra Svinesund i sør til Storskog i nordøst. Trafikken er ikke registrert i selve grenseovergangen, men litt inn på norsk side. Det er ingen store kryss mellom registreringspunktene og riksgrensen, slik at de fleste kjøretøy som blir registrert antas å krysse riksgrensen, selv om enkelte kjøretøy kan stoppe og eventuelt snu på norsk side.

Datagrunnlaget for beregningene hentes fra disse trafikkregistreringspunktene. Det vil gi et representativ utvalg for å kunne beregne trafikkutviklingen. Det gjøres en sammenligning av døgntrafikken, ukedag mot ukedag. Mandag sammenlignes med mandag osv.  

Trafikkutviklingen sammenlignes slik:

* Døgntrafikken sammenlignes med tilsvarende døgntrafikk i uke 10 i 2020.
* Døgntrafikken sammenlignes med tilsvarende døgntrafikk i samme uke i 2019.
* For påsken sammenlignes døgntrafikken mellom tilsvarende påskedager, fra fredag før palmesøndag til andre påskedag.

Alle data er hentet fra [trafikkdata.no](https://www.trafikkdata.no/). Døgntrafikk med minst 99 % dekningsgrad er brukt til å beregne prosentvis endring mellom to dager.

Vi tar forbehold om feil i datagrunnlaget. Enkelte trafikkregistreringspunkter er utelatt på grunn av feil på utstyr eller at de ligger på veier som er påvirket av vegarbeid o.l. Datagrunnlaget inkluderer likevel mange nok punkter til å få fram en generell trend i trafikkutviklingen.

Været vil enkelte dager kunne påvirke trafikkmengden. For eksempel vil det ved dårlig føre, som ved stort snøfall eller underkjølt regn, bli redusert framkommelighet og dette fører til mindre trafikk enn normalt. Spesielt gjelder dette en del av grenseovergangene som ligger i fjellstrøk. Enkelte grenseoverganger er vinterstid tidvis stengt på grunn av værforhold.


# Trafikkregistreringspunkt ved riksgrenseovergangene

```{r trptable}
border_trp_adt %>%
  select(-trp_id, -lat, -lon) %>% 
  flextable() %>%
  set_header_labels(name = "Punktnavn",
                    road_reference = "Vegsystemreferanse",
                    adt = "ÅDT",
                    municipality_name = "Kommune") %>%
  bold(part = "header") %>%
  fontsize(size = 9, part = "all") %>%
  font(fontname = "Lucida Sans Unicode", part = "all") %>%
  bg(bg = "#ED9300", part = "header") %>%
  border_remove() %>%
  hline_top(part = "header", border = borderline) %>%
  hline_bottom(part = "all", border = borderline) %>%
  colformat_num(j = 4, na_str = "", digits = 0,
                big.mark = " ") %>% 
  autofit() %>%
  height_all(height = .2) %>%
  padding(padding.top = .3,
          padding.bottom = .3) %>%
  set_caption("Trafikkregistreringspunkter ved riksgrenseovergangene.")
```


# Samlet endring i trafikk over riksgrensen

## Sammenlignet med uke 10 i 2020
Påsken, som her er definert til å gjelde fra og med fredag før palmesøndag til og med Andre påskedag, er tatt ut og sammenlignet i egen figur og tabell.

Spesielle dager i mai og juni tas ut av den direkte sammenligningen uke for uke. Spesielle dager må sammenlignes direkte, og dette gjelder 1. mai, 17. mai, Kristi himmelfartsdag med påfølgende helg og Pinsen.

Med uke 20 avsluttes rekken med sammenligninger med uke 10, da sesongvariasjoner normalt gjør at det ikke forventes samme mengde trafikk.

```{r plot_202011, fig.width=8, fig.height=8}
make_daily_border_plot(border_base_2020, "2020")
```
\
```{r table_2020}
make_border_table(border_base_2020,
                  "Prosentvis endring i trafikkmengde fra uke 10 i 2020.")
```
\
I tabellen over er endringen i trafikkmengden oppgitt i prosent, sammenlignet med uke 10 som utgangspunkt. Det vil si at trafikken har gått ned fra uke 10 til uke 11, når prosentverdien er negativ.


## Sammenlignet mellom samme uke i 2019 og 2020
Påsken, som her er definert til å gjelde fra og med fredag før palmesøndag til og med Andre påskedag, er tatt ut og sammenlignet i egen figur og tabell lenger ned. Siden påsken i 2019 var i uke 16, mens den i 2020 var i uke 15, blir det to uker som ikke kan sammenlignes direkte mellom årene.

Uke 21 i 2020 hadde Kristi himmelfartsdag, og er her sammenlignet med tilsvarende uke i 2019 som var uke 22. Den er angitt med "Uke 21/22".


```{r plot_201911, fig.width=8, fig.height=8}
make_daily_border_plot(border_base_2019, "Fra 2019 til 2020")
```
\
```{r table_2019}
make_border_table(border_base_2019,
                  "Prosentvis endring i trafikkmengde fra 2019 til 2020.")
```
\
I tabellen over er endringen i trafikkmengden oppgitt i prosent, sammenlignet med uke 11 i 2019 som utgangspunkt. Det vil si at trafikken har gått ned fra år 2019 til samme ukedag i 2020, når prosentverdien er negativ. 

```{r plot_201911_part_2, fig.width=8, fig.height=8}
#make_daily_border_plot(border_base_2019_part_2, "Fra 2019 til 2020")
```

```{r table_2019_part_2}
#make_border_table(border_base_2019_part_2,
#                  "Prosentvis endring i trafikkmengde fra 2019 til 2020.")
```



## Sammenligning av spesielle dager i mai

```{r plot_spring, fig.width=8, fig.height=8}
make_daily_border_plot(border_dt_spring_specials, "Fra 2019 til 2020")
```
\
```{r table_spring}
make_border_table(border_dt_spring_specials,
                  "Prosentvis endring i trafikkmengde fra 2019 til 2020.")
```

## Sammenligning av påsketrafikken

```{r plot_easter, fig.width=8, fig.height=8}
make_daily_border_plot(border_easter, "Fra 2019 til 2020")
```
\
```{r table_easter}
#make_border_table(border_easter,
#                  "Prosentvis endring i trafikkmengde fra påsken 2019 til påsken 2020.")

border_base_table_ready_easter <- border_easter %>%
  dplyr::ungroup() %>% 
  dplyr::select(-no_trp) %>%
  dplyr::mutate(index = dplyr::na_if(index, "Inf")) %>% 
  tidyr::pivot_wider(names_from = ukedag,
                     values_from = index) %>% 
  dplyr::select(1, Mandag, everything())

ncol_dt_easter <- ncol(border_base_table_ready_easter)

border_base_table_ready_easter %>% 
  select(periode, Mandag, Tirsdag, Onsdag, Torsdag, Fredag, Lørdag, Søndag) %>% 
  flextable() %>%
  set_header_labels(periode = "Sammenlignet") %>%
  bold(part = "header") %>%
  fontsize(size = 9, part = "all") %>%
  font(fontname = "Lucida Sans Unicode", part = "all") %>%
  bg(bg = "#ED9300", part = "header") %>%
  border_remove() %>%
  hline_top(part = "header", border = borderline) %>%
  hline_bottom(part = "all", border = borderline) %>%
  colformat_num(j = 2:ncol_dt_easter, na_str = "", digits = 1) %>% 
  autofit() %>%
  height_all(height = .2) %>%
  padding(padding.top = .3,
          padding.bottom = .3) %>%
  set_caption("Prosentvis endring i trafikkmengde fra påsken 2019 til påsken 2020.")
```


# Endring i trafikk ved de enkelte riksgrenseovergangene

## Sammenlignet med uke 10 i 2020

```{r trptable_202010}
# border_trp_dt_2020_10_11 %>%
#   select(-road, -adt, -municipality_name) %>% 
#   flextable() %>%
#   set_header_labels(name = "Punktnavn") %>%
#   bold(part = "header") %>%
#   fontsize(size = 10, part = "all") %>%
#   font(fontname = "Lucida Sans Unicode", part = "all") %>%
#   bg(bg = "#ED9300", part = "header") %>%
#   border_remove() %>%
#   hline_top(part = "header", border = borderline) %>%
#   hline_bottom(part = "all", border = borderline) %>%
#   colformat_num(j = 2:8, na_str = "", digits = 1) %>% 
#   autofit() %>%
#   height_all(height = .2) %>%
#   padding(padding.top = .3,
#           padding.bottom = .3) %>%
#   set_caption("Prosentvis endring i trafikkmengde fra uke 10 til uke 11 i 2020.")

make_border_trp_table(border_trp_dt_2020_10_11,
                      "Prosentvis endring i trafikkmengde fra uke 10 til uke 11 i 2020.")
```
\
```{r trptable_202012}
make_border_trp_table(border_trp_dt_2020_10_12,
                      "Prosentvis endring i trafikkmengde fra uke 10 til uke 12 i 2020.")
```
\
```{r trptable_202013}
make_border_trp_table(border_trp_dt_2020_10_13,
                      "Prosentvis endring i trafikkmengde fra uke 10 til uke 13 i 2020.")
```


```{r trptable_202014}
make_border_trp_table(border_trp_dt_2020_10_14,
                      "Prosentvis endring i trafikkmengde fra uke 10 til uke 14 i 2020.")
```


```{r trptable_202016}
make_border_trp_table(border_trp_dt_2020_10_16,
                      "Prosentvis endring i trafikkmengde fra uke 10 til uke 16 i 2020.")
```


```{r trptable_202017}
make_border_trp_table(border_trp_dt_2020_10_17,
                      "Prosentvis endring i trafikkmengde fra uke 10 til uke 17 i 2020.")
```


```{r trptable_202018}
make_border_trp_table(border_trp_dt_2020_10_18,
                      "Prosentvis endring i trafikkmengde fra uke 10 til uke 18 i 2020. Fredag er ikke med da det var 1. mai i uke 18.")
```


```{r trptable_202019}
make_border_trp_table(border_trp_dt_2020_10_19,
                      "Prosentvis endring i trafikkmengde fra uke 10 til uke 19 i 2020.")
```

```{r trptable_202020}
make_border_trp_table(border_trp_dt_2020_10_20,
                      "Prosentvis endring i trafikkmengde fra uke 10 til uke 20 i 2020. Søndag 17. mai er utelatt.")
```



## Sammenlignet mellom samme uke i 2019 og 2020

```{r trptable_201911}
make_border_trp_table(border_trp_dt_201911_202011,
                      "Prosentvis endring i trafikkmengde fra uke 11 2019 til uke 11 2020.")
```


```{r trptable_201912}
make_border_trp_table(border_trp_dt_201912_202012,
                      "Prosentvis endring i trafikkmengde fra uke 12 2019 til uke 12 2020.")
```


```{r trptable_201913}
make_border_trp_table(border_trp_dt_201913_202013,
                      "Prosentvis endring i trafikkmengde fra uke 13 2019 til uke 13 2020.")
```

```{r trptable_201914}
make_border_trp_table(border_trp_dt_201914_202014,
                      "Prosentvis endring i trafikkmengde fra uke 14 2019 til uke 14 2020.")
```

```{r trptable_201917}
make_border_trp_table(border_trp_dt_201917_202017,
                      "Prosentvis endring i trafikkmengde fra uke 17 2019 til uke 17 2020.")
```


```{r trptable_201918}
make_border_trp_table(border_trp_dt_201918_202018,
                      "Prosentvis endring i trafikkmengde fra uke 18 2019 til uke 18 2020. Dagene som var 1. mai - onsdag i 2019 og fredag i 2020, er tatt ut.")
```


```{r trptable_201919}
make_border_trp_table(border_trp_dt_201919_202019,
                      "Prosentvis endring i trafikkmengde fra uke 19 2019 til uke 19 2020.")
```

```{r trptable_201920}
make_border_trp_table(border_trp_dt_201920_202020,
                      "Prosentvis endring i trafikkmengde fra uke 20 2019 til uke 20 2020. 17. mai er utelatt.")
```

```{r trptable_201921}
make_border_trp_table(border_trp_dt_2019_2020_ascension,
                      "Prosentvis endring i trafikkmengde fra uke 22 2019 til uke 21 2020.")
```


```{r trptable_201922}
make_border_trp_table(border_trp_dt_2019_2020_whitsun,
                      "Prosentvis endring i trafikkmengde fra uke 23 2019 til uke 22 2020. Lørdag og søndag er henholdsvis Pinseaften og 1. pinsedag.")
```

```{r trptable_201923}
make_border_trp_table(border_trp_dt_2019_2020_whitmonday,
                      "Prosentvis endring i trafikkmengde fra uke 24 2019 til uke 23 2020. Mandag er 2. pinsedag.")
```

```{r trptable_201924}
make_border_trp_table(border_trp_dt_201921_202024,
                      "Prosentvis endring i trafikkmengde fra uke 21 2019 til uke 24 2020. Uke 24 er første uke uten spesielle dager, og kan best sammenlignes med uke 21 i 2019.")
```


```{r trptable_201925}
# make_border_trp_table(border_trp_dt_201925_202025,
#                      "Prosentvis endring i trafikkmengde fra uke 25 2019 til uke 25 2020.")
```

```{r trptable_201926}
# make_border_trp_table(border_trp_dt_201926_202026,
#                      "Prosentvis endring i trafikkmengde fra uke 26 2019 til uke 26 2020.")
```

```{r trptable_201927}
# make_border_trp_table(border_trp_dt_201927_202027,
#                      "Prosentvis endring i trafikkmengde fra uke 27 2019 til uke 27 2020.")
```

```{r trptable_201928}
# make_border_trp_table(border_trp_dt_201928_202028,
#                      "Prosentvis endring i trafikkmengde fra uke 28 2019 til uke 28 2020.")
```

```{r trptable_201929}
# make_border_trp_table(border_trp_dt_201929_202029,
#                      "Prosentvis endring i trafikkmengde fra uke 29 2019 til uke 29 2020.")
```

```{r trptable_201930}
# make_border_trp_table(border_trp_dt_201930_202030,
#                      "Prosentvis endring i trafikkmengde fra uke 30 2019 til uke 30 2020.")
```

```{r trptable_201931}
# make_border_trp_table(border_trp_dt_201931_202031,
#                      "Prosentvis endring i trafikkmengde fra uke 31 2019 til uke 31 2020.")
```

```{r trptable_201932}
# make_border_trp_table(border_trp_dt_201932_202032,
#                      "Prosentvis endring i trafikkmengde fra uke 32 2019 til uke 32 2020.")
```

```{r trptable_201933}
# make_border_trp_table(border_trp_dt_201933_202033,
#                      "Prosentvis endring i trafikkmengde fra uke 33 2019 til uke 33 2020.")
```

```{r trptable_201934}
# make_border_trp_table(border_trp_dt_201934_2020334,
#                      "Prosentvis endring i trafikkmengde fra uke 34 2019 til uke 34 2020.")
```

```{r trptable_201935}
# make_border_trp_table(border_trp_dt_201935_202035,
#                      "Prosentvis endring i trafikkmengde fra uke 35 2019 til uke 35 2020.")
```


## Sammenligning av spesielle dager i mai

```{r trptable_spring}
make_border_trp_table(border_trp_dt_spring_specials,
                      "Prosentvis endring i trafikkmengde fra 2019 til 2020.")
```



<!-- ADD NEW TABLE HERE!!! -->


## Sammenligning av påsketrafikken

```{r trptable_palmweekend}
make_border_trp_table(border_trp_dt_palmweekend,
                      "Prosentvis endring i trafikkmengde fra palmehelga 2019 til palmehelga 2020.")
```


```{r trptable_easterweek}
make_border_trp_table(border_trp_dt_easterweek,
                      "Prosentvis endring i trafikkmengde fra påskeuka 2019 til påskeuka 2020.")
```


```{r trptable_eastermonday}
make_border_trp_table(border_trp_dt_eastermonday,
                      "Prosentvis endring i trafikkmengde fra 2. påskedag 2019 til 2. påskedag 2020.")
```





