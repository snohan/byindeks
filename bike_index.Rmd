...
index_month <- 8
index_year <- 2021
index_2021_ngl <- get_published_index_for_months(5953, index_year, index_month)
index_2021_grl <- get_published_index_for_months(4953, index_year, index_month)
index_2021_brg <- get_published_index_for_months(5952, index_year, index_month)
index_2021_osl <- get_published_index_for_months(6953, index_year, index_month)
index_2021_njr <- get_published_index_for_months(6952, index_year, index_month)
index_2021_frd <- get_published_index_for_months(9952, index_year, index_month)
...
#write.csv2(index_all_years, file = "sykkelindekser_2018-aug_2021.csv",
#           row.names = F)

#last_month <- max(index_all_years$month_object)
#number_of_cities <- unique(index_all_years$area_name) %>% 
#  length()
...
## Per år
Sykkelindeksene har ulike startår, men i figuren nedenfor vises årlig utvikling per område.

<\br>

```{r yearplot, fig.width=6.5, fig.height=6, fig.cap="Estimert årlig endring i sykkeltrafikk i utvalgte byområder."}
index_all_years %>% 
...
index_all_years %>% 
  ...
  padding(padding.top = .3,
          padding.bottom = .3) %>%
  set_caption("Estimert samlet endring i sykkeltrafikk sammenlignet med foregående år.",
...
## Per måned
I figuren nedenfor vises månedlig utvikling per område. Tall for en måned er sammenlignet med den samme måneden året før.
...
```{r monthplot, fig.width=6.5, fig.height=6, fig.cap="Estimert månedlig endring i sykkeltrafikk i utvalgte byområder."}
index_all_years %>% 
  dplyr::filter(period == "month") %>% 
