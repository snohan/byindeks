...
municipalities <- get_municipalities() %>% 
  dplyr::left_join(counties, by = "county_number")
...
pointindex <- get_published_pointindex_for_months_paginated(2952, 2021, 12)
```
...
Fra mars 2020 gikk grensetrafikken betydelig ned som følge av tiltak mot spredningen av det nye koronaviruset. 6. oktober 2021 ble grenseovergangene gjenåpnet. Smitteverntiltak ble deretter gjeninnført i desember 2021.
...
## Trafikkutvikling siden 2016
Riksgrenseindeksen er beregnet siden 2016, og tabellene nedenfor viser kumulativ trafikkutvikling for hele perioden. Tallene er her beregnet ved å sette basisåret til 100, og den prosentvise utviklingen vil en da finne ved å trekke fra 100.

<br>

```{r historic_per_year}
historic <- index_complete_years %>% 
  dplyr::filter(
     day_type == "ALL",
...
historic_all_combinations %>% 
  ...
  padding(padding.top = .3,
          padding.bottom = .3) %>%
  set_caption("Kumulativ trafikkutvikling på riksgrenseovergangene fra 2016 til 2020.",
               autonum = table_numbers,
               style = "Tabelltekst")
```
...
historic_light_combinations %>% 
  ...
  padding(padding.top = .3,
          padding.bottom = .3) %>%
  set_caption("Kumulativ trafikkutvikling for lette kjøretøy på riksgrenseovergangene fra 2016 til 2020.",
               autonum = table_numbers,
               style = "Tabelltekst")
```
...
historic_heavy_combinations %>% 
  ...
  padding(padding.top = .3,
          padding.bottom = .3) %>%
  set_caption("Kumulativ trafikkutvikling for tunge kjøretøy på riksgrenseovergangene fra 2016 til 2020.",
               autonum = table_numbers,
               style = "Tabelltekst")
```
