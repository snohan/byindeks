...
index_2022 <- get_published_index_for_months(2952, 2022, latest_published_month)
...
pointindex_2022 <- get_published_pointindex_for_months_paginated(2952, 2022, latest_published_month)
```

```{r prepare_pointindex, include=FALSE}
pointindices <- 
  dplyr::bind_rows(
    pointindex_2017[[2]],
    pointindex_2018[[2]],
...
border_crossings_adt %>%
  ...
  set_header_labels(
    road_category_and_number = "Veg-nummer",
    street_name = "Vegnavn",
    #county_name = "Fylke",
...
I kartet nedenfor vises plasseringen av trafikkregistreringspunktene. Grenseovergangene med mest trafikk er Ev 6 og Fv 118 ved Svinesund, Ev 18 ved Ørje og Rv 2 ved Magnor.
...
```{r lineplot_historic_1}
#| fig.width = 6,
#| fig.height = 6.2,
#| fig.cap = "Riksgrenseindeks per måned mellom to påfølgende år i perioden 2016-2019.", 
#| cache = FALSE
...
I figuren ovenfor er det tydelig at de største endringene skyldes Påskens vekslende plassering i kalenderen mellom mars og april i påfølgende år.

<br>

```{r lineplot_historic_2}
#| fig.width = 6,
#| fig.height = 6.2,
...
Tabellene på de neste sidene viser tallgrunnlaget for figuren ovenfor. Indekstallene er en estimert samlet endring i trafikkmengde per måned, sammenlignet med samme måned året før. Usikkerheten i estimatet er synliggjort med standardfeil og standardavvik. Standardfeilen sier noe om hvor nære indeksen (som er et estimat) er den faktiske endringen i trafikken, mens standardavviket sier noe om hvor ulik trafikkendringen var på de ulike trafikkregistreringspunktene.


```{r monthly_index_table_function}
create_monthly_index_table <- function(index_df) {
  ...
  index_df %>%
    ...
    fix_border_issues() %>% 
    set_caption(
      paste0("Riksgrenseindeks per måned, ", this_year, "."),
      autonum = table_numbers,
      style = "Tabelltekst"
    )
}
...
historic_all_combinations %>% 
  ...
  padding(padding.top = .3,
          padding.bottom = .3) %>%
  set_caption("Kumulativ trafikkutvikling fra 2016 til 2021.",
               autonum = table_numbers,
               style = "Tabelltekst")
```
...
historic_light_combinations %>% 
  ...
  padding(padding.top = .3,
          padding.bottom = .3) %>%
  set_caption("Kumulativ trafikkutvikling for lette kjøretøy fra 2016 til 2021.",
               autonum = table_numbers,
               style = "Tabelltekst")
```
...
historic_heavy_combinations %>% 
  ...
  padding(padding.top = .3,
          padding.bottom = .3) %>%
  set_caption("Kumulativ trafikkutvikling for tunge kjøretøy fra 2016 til 2021.",
               autonum = table_numbers,
               style = "Tabelltekst")
```
