...
index_examples_cmdt_bergen <- readr::read_rds("representativity/rolling_cmdt_index_bergen.rds") |>
  dplyr::bind_rows()
...
dplyr::inner_join(
  ...
  dplyr::mutate(
    ...
    official = base::abs(ci_upper - ci_lower) /2
  ) |> 
  dplyr::select(x_label, official),
  index_examples_cmdt_bergen |> 
  dplyr::filter(window_years == "one") |> 
  dplyr::mutate(improved = base::abs(ci_upper - ci_lower) /2) |> 
  dplyr::select(x_label, improved),
  by = dplyr::join_by(x_label)
) |> 
tidyr::pivot_longer(
  cols = c(official, improved),
  names_to = "version"
) |> 
dplyr::mutate(
  x_label = as.factor(x_label) |> forcats::fct_inorder()
) |> 
ggplot(aes(x_label, value, color = version)) +
geom_point() +
theme_light() +
    theme(
...
dplyr::inner_join(
  ...
  dplyr::select(x_label, official = n_trp),
  index_examples_cmdt_bergen |> 
  dplyr::filter(window_years == "one") |> 
  dplyr::select(x_label, improved = n_trp),
  by = dplyr::join_by(x_label)
) |> 
tidyr::pivot_longer(
  cols = c(official, improved),
...
indexes_official_bergen |> 
  ...
  visualize_city_36_mdt_index(
    "Data: Statens vegvesen",
    "Estimert endring i trafikkmengde siste glidende 3 år, nåværende metode",
    paste0("Sammenlignet med 2018")
  ) +
  theme(
    plot.background = element_rect(fill = svv_background_color),
