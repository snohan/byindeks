...
# B er bomstasjon, T er trafikkregistreringspunkt.
...
Kartet i figur @fig-map_trp viser plasseringen av trafikkregistreringspunktene. 

```{r}
#| label: fig-map_trp
#| fig-cap: "Trafikkregistreringspunkt og årsdøgntrafikk. Punktet Meådalen på Ev 14 mellom Stjørdal og Meråker er utelatt fra kartvisningen."
#| fig-scap: "Trafikkregistreringspunkt og årsdøgntrafikk."
#| fig-width: 7
...
Tre års glidende indeks glatter ut trender som er av kortere varighet, men som kan være synlige dersom den glidende indeksen beregnes for kortere perioder. Eksempelvis kan en økende trend i trafikken som har oppstått det siste året bli lite synlig i den tre års glidende indeksen dersom de to første årene var stabile eller hadde nedgang. For å kunne si noe om hvilken utvikling den tre års glidende indeksen kan få framovr i tid, er det nyttig å se tilsvarende glidende indekser for perioder på ett og to år. Dette vises i figur @fig-rolling_indices.
...
visualize_rolling_indices(
  rolling_indices_long,
  caption_credit,
  "Estimert gjennomsnittlig endring siste 1, 2 og 3 år",
  paste0(
    "Sammenlignet med ",
    base_year
  )
...
## Endring per år
Endring i trafikkmengde er også beregnet for hele kalenderår. I stedet for et tre års glidende gjennomsnitt tar denne beregningen utgangspunkt i indeksen mellom to påfølgende år. Indekstall som gjelder for lengre perioder enn to påfølgende år er beregnet som en kjedet indeks av indeksene for alle mellomliggende påfølgende års indekser. En kjedet indeks er en multiplikasjon av indekser på desimalform hvor en indeks på 1 tilsvarer ingen endring.

Tabell @tbl-city_index viser den årlige endringen, samt kjedet indeks for hele perioden.
