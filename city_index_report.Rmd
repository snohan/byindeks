...
all_point_info_long <- all_point_info %>%
  ...
  dplyr::mutate(year = case_when(period == "index_17" ~ "2016-2017",
...
header_row_values_from_year <- c("", "", base_year:(last_year-1), base_year) %>% 
  as.character()

header_row_values_to_year_numbers <- c((base_year+1):last_year, last_year) %>% 
  as.character()

header_row_values_to_year_chr <- paste0("-", header_row_values_to_year_numbers)

header_row_values_to_year <- c("", "", header_row_values_to_year_chr)
