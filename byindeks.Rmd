...
hent_trafikkregistreringsstasjon_for_kommune <- function(kommunenr) {
  ...
  trser <- bind_cols(trs, vegreferanse, koordinater) %>% 
    ...
    select(-Status, -Trafikantgruppe, -fylke, -kommune, -kategori, -nummer,
           -status, -hp, -meter) %>% 
...
kommunepunkter_trondheim <- kommunepunkter %>% 
...
# Trondheim
Tabell over byindekspunktene. TRS står for trafikkregistreringsstasjon.
...
kommunepunkter_trondheim  %>% 
  ...
  select(Navn, Kommune, Veg, Type, ADT, Dekning_17, Dekning_18) %>% 
  arrange(Veg) %>% 
  kable()
```
...
Kartet over Trondheim viser trafikkregistreringsstasjoner i rødt, 
og bomstasjoner i blått.
...
# Klæbu
Ingen foreslåtte byindekspunkter.
...
kommunepunkter_melhus <- kommune_trser %>% 
  filter(Stasjonnr %in% byindekspunkter_trondheim)
...
kommunepunkter_melhus  %>% 
  ...
  left_join(dekning_2017) %>% 
  left_join(dekning_2018) %>% 
  select(Navn, Kommune, Veg, ADT, Dekning_17, Dekning_18) %>% 
  arrange(Veg) %>% 
  kable()
```
...
kommunepunkter_malvik <- kommune_trser %>% 
  filter(Stasjonnr %in% byindekspunkter_trondheim)
...
kommunepunkter_stjordal <- kommune_trser %>% 
  filter(Stasjonnr %in% byindekspunkter_trondheim)
